...
def test_grammar_bitmask_backport_is_version_gated():
    original = getattr(
        StructuredOutputManager,
        patch_structured_output._ORIGINAL_GRAMMAR_BITMASK_ATTR,
        None,
    )
    needs_backport = vllm_version_is("0.27.1") or vllm_version_is("0.28.0")

    if needs_backport:
        assert original is not None
        assert StructuredOutputManager.grammar_bitmask is not original
    else:
